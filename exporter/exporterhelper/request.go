...
// RequestReadyHint is an optional interface that can be implemented by Request to provide a way to indicate
// that the request is ready to be exported. To be used with batchSender.
// Experimental: This API is at the early stage of development and may change without backward compatibility
// until https://github.com/open-telemetry/opentelemetry-collector/issues/8122 is resolved.
type RequestReadyHint interface {
	Request
	// ReadyHint returns whether the request is ready to be exported.
	// This is useful to influence batch flush decisions in batchSender.
	ReadyHint() bool
}
